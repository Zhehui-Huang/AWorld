...
from examples.gaia.agent_collections.search_agent.prompts import SystemPrompt, AgentMessagePrompt
from examples.gaia.agent_collections.search_agent.utils import convert_input_messages, extract_json_from_model_output, estimate_messages_tokens
from examples.gaia.agent_collections.search_agent.common import AgentState, AgentStepInfo, AgentHistory, PolicyMetadata, AgentBrain
...
class SearchAgent(Agent):
    def __init__(self, conf: Union[Dict[str, Any], ConfigDict, AgentConfig], name: str, **kwargs):
        super(SearchAgent, self).__init__(conf=conf, name=name, **kwargs)
    ...
    def reset(self, options: Dict[str, Any] = None):
        super(SearchAgent, self).reset(options)
    ...
    def _build_action_prompt(self) -> str:
        ...
        val = "\n".join([_prompt(v.value) for k, v in SearchAction.__members__.items()])
        return val
    ...
    def policy(self, observation: Observation, info: Dict[str, Any] = None, **kwargs) -> Union[List[ActionModel], None]:
        ...
        try:
            ...
            if not llm_result:
                ...
                return [ActionModel(tool_name=Tools.SEARCH_API.value, action_name="stop")]
            ...
            if self.state.stopped or self.state.paused:
                logger.info('Search gent paused after getting state')
                return [ActionModel(tool_name=Tools.SEARCH_API.value, action_name="stop")]
            ...
            raise RuntimeError("Search agent encountered exception while making the policy.", e)
        finally:
            if llm_result:
        ...
        try:
            ...
            if not output_message or not output_message.content:
                ...
                return output_message, AgentResult(
                    ...
                    actions=[ActionModel(agent_name=self.id(), tool_name='search_api', action_name="stop")])
        except:
        ...
        try:
            ...
            if not actions:
                ...
                return output_message, AgentResult(
                    ...
                    actions=[ActionModel(tool_name='search_api', agent_name=self.id(), action_name="done")]
                )

            for action in actions:
                if "action_name" in action:
                    action_name = action['action_name']
                    search_action = SearchAction.get_value_by_name(action_name)
                    ...
                    for k, v in action.items():
                        search_action = SearchAction.get_value_by_name(k)
                        if not search_action:
                            logger.warning(f"Unsupported action: {k}")

                        action_model = ActionModel(agent_name=self.id(), tool_name='search_api', action_name=k, params=v)
                        result.append(action_model)
                        if k == "done":
                            self._finished = True
    ...
    def _make_history_item(
            self, model_output: AgentResult | None,
            state: Observation,
    ...
    def build_messages_from_trajectory_and_observation(self, observation: Optional[Observation] = None) -> List[BaseMessage]:
        """
        ...
        tool_calls = [
            {
                ...
                'args': {
                    'current_state': {
                        'evaluation_previous_goal': 'Success - I started the search',
                        'memory': 'Starting with the new task. I have completed 1/10 searches',
                        'thought': 'I need to search for information about the topic using Wikipedia first.',
                        'next_goal': 'Search Wikipedia for the main entity',
                    },
                    'action': [{'wiki': {'query': 'artificial intelligence'}}],
                },
                'id': '1',
                'type': 'tool_call',
            }
        ...
        # Add first tool message with "Search started" content
        messages.append(ToolMessage(content='Seach started', tool_call_id='1'))
